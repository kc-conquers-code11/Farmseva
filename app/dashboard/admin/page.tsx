...
type CommunityPostAdmin = {
  ...
  post_type: PostType;
  created_at: string;
  profiles?: { fullname: string } | { fullname: string }[] | null;
};
...
// --- Static mock for outbreaks + NOC (unchanged) ---
...
export default function AdminDashboardPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "outbreaks" | "community" | "broadcast" | "schemes" | "noc" | "content"
  >("overview");
  ...
  // NEW: Moderation queue state
  const [moderationPosts, setModerationPosts] = useState<CommunityPostAdmin[]>(
    []
  );
  const [loadingModeration, setLoadingModeration] = useState(true);
  const [moderationActionId, setModerationActionId] = useState<number | null>(
    null
  );

  // NEW: Schemes analytics state
  ...
  // --- Derived data for schemes charts ---
  const schemeChartData = schemeRows.map((s) => ({
    scheme: s.name,
    applicants: s.applicants_count || 0,
    approved: s.approved_count || 0,
  }));

  const totalApplicants = schemeChartData.reduce(
    (sum, s) => sum + s.applicants,
    0
  );
  const totalApproved = schemeChartData.reduce(
  ...
  // --- Moderation queue: realtime community_posts ---
  useEffect(() => {
    const fetchModerationPosts = async () => {
      setLoadingModeration(true);
      const { data, error } = await supabase
        .from("community_posts")
        .select("id, user_id, content, post_type, created_at, profiles(fullname)")
        .order("created_at", { ascending: false })
        .limit(50);

      if (!error && data) {
        setModerationPosts(data as CommunityPostAdmin[]);
      }
    ...
    const channel = supabase
      .channel("admin-community-moderation")
      .on(
        "postgres_changes",
        ...
        (payload) => {
          if (payload.eventType === "INSERT") {
            setModerationPosts((prev) => [
              payload.new as CommunityPostAdmin,
              ...prev,
            ]);
  ...
  // --- Schemes analytics from schemes table ---
  useEffect(() => {
    const fetchSchemes = async () => {
      setLoadingSchemes(true);
      const { data, error } = await supabase
        .from("schemes")
        .select("id, name, category, applicants_count, approved_count");

      if (!error && data) {
        setSchemeRows(data as SchemeRow[]);
      }
  ...
  // --- Broadcast history from broadcasts table ---
  useEffect(() => {
  ...
  const handleDeleteCommunityPost = async (postId: number) => {
    if (!confirm("Delete this post from community feed?")) return;
    ...
    return (
      <div className="min-h-screen bg-neutral-50 pt-20 text-center flex flex-col items-center justify-center">
        ...
        <button
          ...
          className="mt-6 px-4 py-2 bg-neutral-900 text-white rounded-lg text-sm"
        >
          Go Back
        </button>
      </div>
  ...
  const broadcastTypeLabel = (t: string) => {
    switch (t) {
      case "weather":
        return "Weather";
      case "scheme":
  ...
  const broadcastTypeColor = (t: string) => {
    switch (t) {
      case "weather":
        return "bg-blue-500";
      case "scheme":
                ...
                >
                  <Icon
                    ...
                    className={`w-5 h-5 transition-transform duration-300 ${
                      isActive
                        ? "scale-110"
                        : "group-hover:scale-110 text-neutral-400 group-hover:text-neutral-900"
                    }`}
                  />
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <Card className="border-none shadow-sm ring-1 ring-neutral-100 hover:shadow-lg transition-all">
                  ...
                  <h2 className="text-3xl font-bold text-neutral-800 mb-1">
                    {stats.total}
                  </h2>
                ...
                <Card className="border-none shadow-sm ring-1 ring-neutral-100 hover:shadow-lg transition-all">
                  ...
                  <h2 className="text-3xl font-bold text-neutral-800 mb-1">
                    {mockNocRequests.filter((n) => n.status === "Pending")
                      .length}
                  </h2>
                  <p className="text-xs text-neutral-500 font-medium">
                    NOC Requests Awaiting
                  </p>
                </Card>
                          ...
                          <td className="px-6 py-4">
                            <div className="relative">
                              ...
                              <Icon
                                ...
                                className="absolute right-2 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-400 pointer-events-none"
                              />
                            </div>
                          </td>
              ...
              <Card className="border-none shadow-sm ring-1 ring-neutral-100">
                <div className="flex justify-between items-center mb-8 border-b border-neutral-100 pb-4">
                  ...
                  <span className="px-3 py-1 bg-neutral-900 text-white text-xs font-bold rounded-full">
                    {loadingModeration
                      ? "Loading..."
                      : `${moderationPosts.length} Posts`}
                  </span>
                </div>

                {loadingModeration ? (
                  <div className="flex flex-col items-center justify-center py-10 text-neutral-400 text-sm">
                    <Icon
                      icon="mdi:loading"
                    ...
                    {moderationPosts.map((post) => {
                      ...
                      if (post.profiles) {
                        if (Array.isArray(post.profiles)) {
                          authorName =
                            post.profiles[0]?.fullname || "User";
                        } else {
                          authorName =
                            (post.profiles as any).fullname || "User";
                        }
                      }
                      ...
                      return (
                        <div
                          key={post.id}
                          className="p-5 rounded-2xl bg-white border border-neutral-200 hover:shadow-md transition-shadow"
                        >
                          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                            <div className="flex-1">
                              <div className="flex items-center gap-3 mb-2">
                                ...
                                <span className="text-xs bg-red-50 text-red-600 px-2 py-1 rounded-md font-bold uppercase tracking-wide border border-red-100">
                                  {reasonLabel}
                                </span>
                                <span className="text-xs bg-neutral-50 text-neutral-500 px-2 py-1 rounded-md font-semibold border border-neutral-200">
                            ...
                            <div className="flex gap-2 w-full md:w-auto">
                              <button
                                className="flex-1 md:flex-none px-4 py-2 text-sm font-semibold bg-white border border-neutral-300 rounded-lg hover:bg-neutral-50 text-neutral-700 transition-colors"
                              ...
                              <button
                                ...
                                className="flex-1 md:flex-none px-4 py-2 text-sm font-semibold bg-red-600 text-white rounded-lg hover:bg-red-700 shadow-sm shadow-red-200 transition-colors disabled:opacity-50"
                              >
                                {moderationActionId === post.id
                                  ? "Removing..."
                                  : "Delete Post"}
                  ...
                  <Card className="border-none shadow-sm ring-1 ring-neutral-100">
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-neutral-500">
                          Notify users via SMS & App Notifications
                        </p>
                      </div>
                    </div>
                    ...
                    {broadcastSent ? (
                      <motion.div
                        ...
                        className="bg-green-50 border border-green-200 text-green-800 p-8 rounded-2xl text-center py-12"
                      >
                      ...
                      >
                        ...
                        <button
                          type="submit"
                          className="w-full bg-neutral-900 hover:bg-black text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-all transform hover:-translate-y-1 shadow-lg shadow-neutral-900/20"
                        >
                          <Icon
                            icon="mdi:send"
                            className="w-5 h-5"
                  ...
                  <Card className="border-none shadow-sm ring-1 ring-neutral-100 h-full">
                    ...
                    {loadingBroadcasts ? (
                      ...
                      <div className="relative border-l-2 border-neutral-100 ml-3 space-y-8">
                        {broadcasts.slice(0, 6).map((item, i) => (
                          <div
                            key={item.id}
                            className="relative pl-6"
                          >
                            <div
                              className={`absolute -left-[9px] top-1.5 w-4 h-4 rounded-full border-4 border-white ${broadcastTypeColor(
                                item.type
                              )}`}
                            ></div>
                            <div className="bg-neutral-50 p-3 rounded-lg border border-neutral-100">
                              ...
                              <div className="flex justify-between text-xs text-neutral-500 mt-2 font-medium">
                                <span className="bg-white px-2 py-0.5 rounded border border-neutral-100">
            ...
            >
              {loadingSchemes ? (
                <div className="py-16 text-center text-neutral-400 text-sm">
                  Loading schemes analytics...
                </div>
              ) : (
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <Card className="border-none shadow-sm ring-1 ring-neutral-100">
                      <h3 className="font-bold text-neutral-800 mb-6">
                        Application Volume
                      </h3>
                      <div className="h-64">
                        <ResponsiveContainer
            ...
            >
              <Card className="border-none shadow-sm ring-1 ring-neutral-100 p-0 overflow-hidden">
                <div className="p-6 border-b border-neutral-100 flex justify-between items-center bg-orange-50/30">
                  <h3 className="text-lg font-bold text-neutral-800 flex items-center gap-2">
                    <Icon icon="mdi:file-certificate-outline" /> NOC
                    Verification Queue
                  </h3>
                    ...
                    <tbody className="divide-y divide-neutral-50">
                      {mockNocRequests.map((req) => (
                        <tr
                          ...
                          className="hover:bg-neutral-50 transition-colors"
                        >
                          <td className="px-6 py-4 font-mono text-neutral-400 font-medium">
                            #{req.id}
                          </td>
                          <td className="px-6 py-4">
                          ...
                          <td className="px-6 py-4">
                            {req.status === "Pending" ? (
                              <div className="flex gap-2">
                                <button
                                  className="bg-green-100 text-green-700 hover:bg-green-600 hover:text-white p-2 rounded-lg transition-all"
                                  title="Approve"
                                >
                                  <Icon
                                    icon="mdi:check-bold"
                                ...
                                <button
                                  className="bg-red-100 text-red-700 hover:bg-red-600 hover:text-white p-2 rounded-lg transition-all"
                                  title="Reject"
                                >
                                  <Icon

...
export default function AccessibilityWidget() {
  ...
  useEffect(() => {
    ...
    // 2. Filter Effects (Contrast & Saturation)
    let filterString = ''
  ...
  return (
    <>
      {/* Global Styles for Accessibility Overrides */}
      <style jsx global>{`
            ...
            >
              ...
              <div className="p-5 space-y-6 max-h-[60vh] overflow-y-auto custom-scrollbar">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-neutral-500 uppercase tracking-wider">Text Size</label>
                  <div className="flex items-center justify-between bg-neutral-100 rounded-xl p-1">
                    ...
                    <span className="font-bold text-neutral-800">{settings.fontSize}%</span>
                    <button 
                ...
                {/* Display Mode */}
                <div className="space-y-2">
                  <label className="text-xs font-bold text-neutral-500 uppercase tracking-wider">Display</label>
                  <div className="grid grid-cols-2 gap-2">
                    <button 
                      onClick={() => setSettings(s => ({ ...s, contrast: s.contrast === 'invert' ? 'normal' : 'invert' }))}
                      className={`p-3 rounded-xl border flex flex-col items-center gap-2 transition ${settings.contrast === 'invert' ? 'bg-neutral-900 text-white border-neutral-900' : 'bg-white border-neutral-200 hover:border-neutral-300'}`}
                    >
                      <Moon size={20} /> <span className="text-xs font-medium">Invert Colors</span>
                    </button>
                    <button 
                      onClick={() => setSettings(s => ({ ...s, saturation: s.saturation === 0 ? 100 : 0 }))}
                      className={`p-3 rounded-xl border flex flex-col items-center gap-2 transition ${settings.saturation === 0 ? 'bg-neutral-900 text-white border-neutral-900' : 'bg-white border-neutral-200 hover:border-neutral-300'}`}
                    >
                      <Eye size={20} /> <span className="text-xs font-medium">Grayscale</span>
                    </button>
                  </div>
                </div>
                ...
                {/* Tools */}
                <div className="space-y-2">
                  <label className="text-xs font-bold text-neutral-500 uppercase tracking-wider">Tools</label>
                  <div className="space-y-2">
                    <ToggleButton 
...
function ToggleButton({ active, onClick, icon, label }: any) {
  return (
    ...
    >
      ...
      <div className={`ml-auto w-3 h-3 rounded-full ${active ? 'bg-blue-500' : 'bg-neutral-300'}`}></div>
    </button>
  )
}
